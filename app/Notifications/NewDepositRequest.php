...
class NewDepositRequest extends Notification {
    ...
    public function __construct($depositRequest) {
        ...
        $this->template = EmailTemplate::where('slug', 'DEPOSIT_REQUEST_SUBMITTED')
            ->where('tenant_id', $tenantId)
            ->first();

        $balance  = get_account_balance($this->depositRequest->credit_account_id, $this->depositRequest->member_id);
        $currency = $this->depositRequest->account->savings_type->currency->name;
        ...
        $this->replace['dateTime']       = $this->depositRequest->created_at;
        $this->replace['requestId']      = $this->depositRequest->id;
        $this->replace['description']    = $this->depositRequest->description ?? '';
    }
